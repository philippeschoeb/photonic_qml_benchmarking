...
for model in "${MODELS[@]}"; do
  ...
  for backend in ${backends}; do
    if [[ "${backend}" == "classical" ]]; then
      ...
      run_single_cmd "${model}" "classical" python main.py --dataset "${DATASET}" --model "${model}" --run_type single --random_state "${RANDOM_STATE}" --max_train_time_seconds "${MAX_TRAIN_TIME_SECONDS}" --big_script_name "${BATCH_RUN_GROUP}" "${WANDB_FLAG[@]}"
    else
      echo "-> Model: ${model}, Backend: ${backend}"
      run_single_cmd "${model}" "${backend}" python main.py --dataset "${DATASET}" --model "${model}" --backend "${backend}" --run_type single --random_state "${RANDOM_STATE}" --max_train_time_seconds "${MAX_TRAIN_TIME_SECONDS}" --big_script_name "${BATCH_RUN_GROUP}" "${WANDB_FLAG[@]}"
    fi
  done
done
