...
class MLP(torch.nn.Module):
    def __init__(self, input_size, output_size, numNeurons, **kwargs):
        ...
        assert len(numNeurons) == self.num_layers, f'Number of neurons ({self.num_neurons}) must contain num_h_layers ({self.num_layers}) elements'
        ...
        if self.num_layers == 0:
            ...
            last_linear = torch.nn.Linear(in_features=self.num_neurons[-1], out_features=output_size, bias=True)
