...
class QRKS:
    ...
    """
    def __init__(self, R, gamma, input_size, m, n, circuit, no_bunching, scaling='1/pi', C=1.0, probability=False, random_state=None):
    ...
    def get_kernels(self, x_train, x_test):
        ...
        self.pqc.eval()
        train_input = torch.Tensor(x_r_i_s_train).view(len(x_r_i_s_train) * self.R, -1) * torch.tensor(self.scaling)
        test_input = torch.Tensor(x_r_i_s_test).view(len(x_r_i_s_test) * self.R, -1) * torch.tensor(self.scaling)
        z_s_train = self.pqc(train_input)
        z_s_test = self.pqc(test_input)

        z_s_train = z_s_train.view(len(x_r_i_s_train), self.R)
        z_s_test = z_s_test.view(len(x_r_i_s_test), self.R)

        # In the paper, they multiply by 1/sqrt(R)
        z_s_train = z_s_train * 10
        z_s_test = z_s_test * 10
