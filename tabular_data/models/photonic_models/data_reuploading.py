...
class DataReuploading(MerlinReuploadingClassifier):
    ...
    def __init__(self, input_size, numLayers, design='AA', scaling='1/pi', **kwargs):
        assert input_size <= numLayers * 2, f'Not enough layers ({numLayers}) to encode all the input data of size {input_size}.'
...
class SKDataReuploading(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, x, y):
        ...
        self.model.fit(x, y, track_history=track_history, max_epochs=max_epochs, learning_rate=learning_rate,
                  batch_size=batch_size, patience=patience, tau=tau, convergence_tolerance=convergence_tolerance)
        ...
        num_params = sum(p.numel() for p in self.model.quantum_model.parameters() if p.requires_grad)
