...
class QuantumKitchenSinks(BaseEstimator, ClassifierMixin):
    def __init__(
        ...
        Args:
            ...
            var (float): determined the variance of the matrix `\omega` used to lienarly transform the input
                features.
        ...
        self.linear_model = (
            LogisticRegression(
                penalty="l2", C=1.0, solver="lbfgs", tol=1e-4, max_iter=1000
            )
            if linear_model is None
            else clone(linear_model)
        )
