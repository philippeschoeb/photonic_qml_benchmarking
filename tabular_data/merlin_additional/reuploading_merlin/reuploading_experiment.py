...
class ReUploadCircuitModelTwoMode:
    ...
    Var blocks (consume 2 trainable params per block except D):
      A: PS_0(θ_k) ; BS_0() ; PS_0(φ_k) ; BS_0()
      B: PS_0(θ_k) ; BS_0(φ_k)
      C: PS_0(θ_k) ; PS_1(φ_k) ; BS_0()
      D: PS_0(random) ; BS_0() ; PS_0(random) ; BS_0()
    ...
    def _var_D(self, circuit, data_idx: int, var_idx: int) -> Tuple[int, int]:
        # D: PS_0(random) ; BS() ; PS_0(random) ; BS()
        circuit.add(0, pcvl.PS(phi=random.random() * np.pi), merge=True)
        circuit.add(0, pcvl.BS(), merge=True)
        circuit.add(0, pcvl.PS(phi=random.random() * np.pi), merge=True)
        circuit.add(0, pcvl.BS(), merge=True)
        return data_idx, var_idx
...
class PercevalReuploadingClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        ...
        *,
        ...
        self.classifier_ = LDA1DHead()
        self.is_fitted_ = False
        self.training_history_: Optional[Dict[str, Any]] = None
