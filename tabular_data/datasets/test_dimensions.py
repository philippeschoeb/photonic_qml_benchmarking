...
import downscaled_mnist_pca_loading
import hidden_manifold_loading
...
def test_downscaled_mnist():
    ...
    for d in test_dims:
        try:
            x_train, x_test, y_train, y_test = downscaled_mnist_pca_loading.get_dataset(
                d
            )
            print(
                f"  d={d}: x_train.shape={x_train.shape}, x_test.shape={x_test.shape}"
            )
...
def test_hidden_manifold():
    ...
    for d in test_dims:
        try:
            x_train, x_test, y_train, y_test = hidden_manifold_loading.get_dataset(d, 6)
            print(
                f"  d={d}, m=6: x_train.shape={x_train.shape}, x_test.shape={x_test.shape}"
            )
            print(f"            Expected feature dim: {d}, Actual: {x_train.shape[1]}")
    ...
    for m in test_ms:
        try:
            x_train, x_test, y_train, y_test = hidden_manifold_loading.get_dataset(
                10, m
            )
            print(
                f"  d=10, m={m}: x_train.shape={x_train.shape}, x_test.shape={x_test.shape}"
            )
...
def test_two_curves():
    ...
    for d in test_dims:
        try:
            x_train, x_test, y_train, y_test = two_curves_loading.get_dataset(d, 5)
            print(
                f"  d={d}, degree=5: x_train.shape={x_train.shape}, x_test.shape={x_test.shape}"
            )
            print(
                f"                 Expected feature dim: {d}, Actual: {x_train.shape[1]}"
            )
            assert x_train.shape[1] == d, (
                f"Feature dimension mismatch for d={d}, degree=5"
            )
    ...
    for degree in test_degrees:
        try:
            x_train, x_test, y_train, y_test = two_curves_loading.get_dataset(
                10, degree
            )
            print(
                f"  d=10, degree={degree}: x_train.shape={x_train.shape}, x_test.shape={x_test.shape}"
            )
            print(
                f"                       Expected feature dim: 10, Actual: {x_train.shape[1]}"
            )
            assert x_train.shape[1] == 10, (
                f"Feature dimension mismatch for d=10, degree={degree}"
            )
