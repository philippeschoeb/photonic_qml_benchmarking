                    ...
                    if isinstance(obj[key][subkey], h5py.Group):
                        ...
                        if subkey == 'inputs' and len(obj[key][subkey]) > 0:
                            ...
                            if isinstance(sample_group, h5py.Group):
                                sample_dims = sorted([int(k) for k in sample_group.keys()])
                                print(f"        Sample {sample_key} dimensions: {sample_dims} (count: {len(sample_dims)})")
                    else:
                        print(f"      Shape: {obj[key][subkey].shape if hasattr(obj[key][subkey], 'shape') else 'scalar'}")
            ...
            for d in ['2', '5', '10', '15', '20']:
                if d in train_group:
                    ...
                    if 'inputs' in subgroup:
                        ...
                        print(f"  Inputs keys: {list(inputs.keys())[:5]}... (total: {len(inputs)})")
                        ...
                        if len(inputs) > 0:
                            ...
                            if isinstance(first_sample, h5py.Group):
                                ...
                                print(f"  First sample dimensions: {dims} (count: {len(dims)})")
                            else:
