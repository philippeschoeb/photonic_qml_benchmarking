...
def get_batch(X, y, rnd_key, batch_size=32):
    ...
    # convert to numpy
    rnd_indices = jnp.array(rnd_indices)  # already jnp array
    rnd_indices = jax.device_get(rnd_indices)  # copy to host as np array
...
def chunk_grad(grad_fn, max_vmap):
    ...
    def chunked_grad(params, X, y, key):
        ...
        # Use tree_map to average gradients for each leaf
        def average_grads(*leaves):
            return jnp.mean(jnp.stack(leaves, axis=0), axis=0)

        grad_dict = jax.tree_util.tree_map(average_grads, *grads)
        return grad_dict
